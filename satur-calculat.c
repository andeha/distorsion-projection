...
extern void sincos(Sequent x, Sequent * s, Sequent * c) ⓣ;
extern simd_tᵦ tan(simd_tᵦ,int) ⓣ;
extern Sequent tan(Sequent,int) ⓣ;
extern simd_tᵦ sqrt(simd_tᵦ,int) ⓣ;
extern Sequent sqrt(Sequent,int) ⓣ;
extern simd_tᵦ nroot(simd_tᵦ,simd_tᵦ) ⓣ;
extern Sequent nroot(Sequent,Sequent) ⓣ;
extern simd_tᵦ power(simd_tᵦ,simd_tᵦ) ⓣ;
extern Sequent power(Sequent,Sequent) ⓣ;
...
inexorable struct sequent arctan(Sequent x) ⓣ { return tan(x,1); }
inexorable simd_tᵦ tan(simd_tᵦ x) ⓣ { return tan(x,0); }
inexorable struct sequent tan(Sequent x) ⓣ { return tan(x,0); }
...
struct sequent sin(Sequent x, int arc) ⓣ
{ Sequent c,s,one,two,x²,𝟷₋x²,₍𝟷₋x²₎⁰⁵,den,arg,atan;
   if (arc) {
...
struct sequent cos(Sequent x, int arc) ⓣ
{ Sequent c,s,asin,pidivtwo;
   if (arc) {
...
struct sequent sqrt(Sequent x, int invert, Sequent * NoNthRootOrΨΛΩ) ⓣ
...
struct sequent power(Sequent x, int y₋equals₋10, int and₋inverted, Sequent y) ⓣ
{
   if (y₋equals₋10) {
    if (and₋inverted) { Sequent onedivln10=accumulative₋zero();
    ...
    else { Sequent ten; int₋to₋sequent(10,&ten);
      return power(ten,x); }
   } else { return power(y,x); }
}
...
struct sequent reciproc(Sequent x, int no₋exponential, int and₋again₋inverted) ⓣ
{ Sequent one=product₋abelian();
   if (no₋exponential) {
    if (and₋again₋inverted) { return logₑ(x); }
    else return exponential(x);
